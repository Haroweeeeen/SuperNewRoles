            ...
            if (ModeHandler.IsMode(ModeId.SuperHostRoles))
            {
                switch (__instance.GetRole())
                {
                    ...
                    case RoleId.Camouflager:
                        ...
                        return true;
                    case RoleId.Moving:
                        if (!RoleClass.Moving.Seted.ContainsKey(__instance.PlayerId))
                        {
                            RoleClass.Moving.Seted[__instance.PlayerId] = true;
                            RoleClass.Moving.setpostionSHR[__instance.PlayerId] = new Vector3(__instance.transform.position.x, __instance.transform.position.y, __instance.transform.position.z);
                        }
                        else
                        {
                            __instance.RpcSnapTo(RoleClass.Moving.setpostionSHR[__instance.PlayerId]);
                        }
                        return false;
                }
            }
